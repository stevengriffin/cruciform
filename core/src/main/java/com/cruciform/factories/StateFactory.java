...
public class StateFactory {
	...
	public static State setState(final Class<? extends State> state, @NotNull final Cruciform game) {
		State result = getOrCreate(state, game, false);
		game.setScreen(result);
		return result;
	}
	
	public static State setAndRenewState(final Class<? extends State> state, @NotNull final Cruciform game) {
		State result = getOrCreate(state, game, true);
		game.setScreen(result);
		return result;
	}
	
	private static State getOrCreate(final Class<? extends State> state, @NotNull final Cruciform game,
